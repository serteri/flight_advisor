    ...
    try {
        ...
        await persistFlightSearchRecords(
            allFlights.map((flight: any) => ({
                ...
                departureTime: String(flight.departTime || flight.departureTime || flight.segments?.[0]?.departure || departureDate),
                arrivalTime: String(flight.arriveTime || flight.arrivalTime || flight.segments?.[flight.segments?.length - 1]?.arrival || departureDate),
                ...
                segments: Array.isArray(flight.segments) && flight.segments.length > 0
                    ? flight.segments
                    : [{
                        from: String(flight.from || from),
                        to: String(flight.to || to),
                        departureTime: String(flight.departTime || flight.departureTime || departureDate),
                        arrivalTime: String(flight.arriveTime || flight.arrivalTime || departureDate),
                        duration: Number(flight.duration) || 0,
                        carrier: String((flight.airlineCode || 'XX')).toUpperCase(),
                        flightNumber: String(flight.flightNumber || 'UNKNOWN'),
                    }],
