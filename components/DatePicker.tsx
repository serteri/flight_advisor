    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            >
                ...
                {variant === "ghost" && label && (
                    <div className="text-xs font-semibold text-slate-500 tracking-[0.04em] mb-1 leading-none pointer-events-none truncate w-full">
                        {label}
                    </div>
                )}
                    ...
                    {selected
                        ? (
                            <div className={`flex flex-col items-start justify-center min-w-0 w-full ${variant === "ghost" ? "gap-0.5" : ""}`}>
                                ...
                                <span className={`text-slate-900 leading-tight truncate w-full whitespace-nowrap ${variant === "ghost" ? "text-base font-semibold" : "text-lg font-bold"}`}>
                                    {format(selected, variant === "ghost" ? "d MMM ''yy" : "d MMMM yyyy", { locale: dateLocale })}
                                </span>
                            </div>
                        )
                        : <span className={`${variant === "ghost" ? "text-base font-semibold truncate w-full whitespace-nowrap" : "text-lg font-semibold"} text-slate-400`}>{placeholder || t('placeholder')}</span>
                    }
                </div>
            </button>
