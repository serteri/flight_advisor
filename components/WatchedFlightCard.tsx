...
import { Plane, Calendar, Clock, ArrowRight, RefreshCcw, TrendingDown, TrendingUp, Minus } from "lucide-react";
...
export function WatchedFlightCard({ flight }: WatchedFlightCardProps) {
    const t = useTranslations("FlightSearch"); // Re-using existing translations where possible
    ...
    const sparklinePoints = history.map((h: any, i: number) => {
        // Normalize 0-100 for height
        // Only if we have history
        return h.price;
    });

    // Simple Render Logic for Sparkline (if history exists)
    // 50px height, 100px width
    let sparklinePath = "";
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4 mt-2">
                ...
                <div className="text-right mt-4">
                    <div className="text-xl font-black text-slate-900">
                        {Math.round(flight.currentPrice).toLocaleString()}
            ...
            <div className="flex items-center gap-4 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-100/50">
                ...
                <div className="flex-1 flex flex-col items-center">
                    <div className="text-[10px] text-slate-400 font-medium">{flight.stops === 0 ? t('nonstop') : `${flight.stops} Stop`}</div>
            ...
            <div className="flex justify-between items-end border-t border-slate-100 pt-3">
                <div className="flex-1 pr-4">
                    <div className="text-[10px] text-slate-400 mb-1 font-medium tracking-wide uppercase">Price History (7 Days)</div>
                    {sparklinePath ? (
